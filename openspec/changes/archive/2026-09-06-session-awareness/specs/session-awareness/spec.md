> **Live-verified scenarios.** Most scenarios below are verified by the test suite alone.
> These are additionally verified against the live estate or the deployed instance, and the
> verifying task is named beside each. A scenario not listed here has no live check and
> must not be described as one.
>
> - *Dry run is inert*, *Both gates admit*, *Root admits, owner refuses*, *Foreground path
>   denies the session* — task **8.5** (`--dry-run` against the live estate, findings
>   recorded as counts in `notes/tier-w-publisher-review.md`).
> - *Unchanged past heartbeat*, *Status change publishes* — task **8.7** (timer enabled;
>   one snapshot observed on the topic through the admin account, a heartbeat re-publish
>   with no change, and a status change within one tick).
> - *Fresh headline*, *Stale headline*, *G2 — nothing published within the lookback* —
>   task **9.2** (asked over Signal against the deployed container; the timer is stopped
>   for 35 minutes to produce the stale case).
> - *Absent section yields defaults* is exercised live only in the negative: task **8.8**
>   sets `sessions.enabled` and the label allowlist together on rp5 and confirms the tool
>   registers.

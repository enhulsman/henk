...
**Reason**: Deliberate, owner-blessed reversal per NORTH-STAR.md (settled 2026-08-07): the roadmap's first change ships production mutating tools (`capture`, `store_memory`) at the standing tier, so "v1 SHALL ship no mutating tools in the production tool registry" no longer describes the intended system.
**Migration**: The test-coverage obligation is carried forward and extended by "Gate paths are covered by automated tests"; the production registry now legitimately contains mutating tools, each carrying an explicit authorization tier and turn scope per the modified classification requirement.

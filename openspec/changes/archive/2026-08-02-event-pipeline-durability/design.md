...
`EventStreamError` now carries the HTTP `status`, and a 400 **with a `since` we could be blamed for** (not a cold subscribe, not the sentinel) triggers recovery: log at ERROR, set the cursor to ntfy's `all` sentinel, notify the owner, and reconnect immediately — the sentinel is valid, so no backoff is needed, and a rejection *of* the sentinel falls through to the normal backoff rather than spinning.

**The recovery is bounded to one immediate, notifying attempt per process** (scrutiny MAJOR). The reasoning above covers a *seeded* bad checkpoint, which is inherently one-shot — but the predicate fires on any rejected `_last_id`, so a cursor that keeps being rejected would loop: replay the full window → events flow → new real id → 400 → notify again, paced only by the server. Measured at roughly one cycle per second. It never wedges, so D8's goal held, but two second-order effects bite: the owner is DM-stormed by the very alert meant to prevent silence, and each replay re-suppresses the whole retained cache, writing one suppression record per event into an audit log this change explicitly does not rotate — unbounded growth on a backed-up volume. So a `_recoveries` counter now gates both the notice and the immediate reconnect: the first recovery behaves as described, later ones set the sentinel silently and take the normal backoff.

Scoping stays at **400 only**, deliberately. The two error directions are asymmetric: a false positive (say a proxy 400 misread as a bad cursor) costs a bounded over-replay with zero events lost — the same benign path natural eviction takes — while a false negative costs total, silent, permanent intake death. Broadening to 404/416/422 buys nothing against the measured contract and widens the false-positive surface; 401/403 must keep falling through to backoff, since a revoked token is not a bad cursor. The residual proxy risk is addressed by the throttle above, not by narrowing the predicate. (Not currently a live concern: Henk reaches ntfy directly over the tailnet at `vps:2586`, no proxy in the path.)

One honest caveat: a fallback can **transiently rewind the persisted cursor**. `_advance_checkpoint` writes blindly — correctly, since ntfy ids are opaque and cannot be ordered — so if a replay delivers older events first, the next batch's offset may be behind what was stored. It walks forward again through the replay, so it self-corrects; a crash inside that window costs one more full replay on restart. Bounded and benign, but the cursor is not strictly monotonic and the README's D1 prose should not be read as claiming otherwise.

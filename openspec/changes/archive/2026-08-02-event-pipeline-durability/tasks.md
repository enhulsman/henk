...
- [x] 4.2 **CLOSED 2026-08-02 together with henk-events 5.4 — see that task for the full evidence.** Outcome: the watch window elapsed with **zero real events** in 9 days (07-24 20:22 → 08-02), so there is no audit data to tune from and the D6 defaults are retained unchanged. The silence was verified genuine per trigger family (Gatus failures all isolated singles below `failure-threshold: 2`; all four Grafana rules correctly inactive — swap peaked 86.3% against a `>90%` threshold, disk 68.8% against `>85%`), and intake was confirmed continuously subscribed throughout. Note for anyone re-running this: the query below **cannot be run as written** — `docker exec` requires PASSWD sudo on rp5, so it is owner-run; `sudo -n docker logs` is the read-only substitute and was sufficient here. Also, Grafana rule thresholds should be read from `~/.claude-config/provisioning/grafana-henk-events.sh`, not the Grafana API (401, no scoped token). Original query:
  ```
...
- [x] 4.3 **DONE 2026-08-02**, immediately after henk-events, in the required order. Original: `/opsx:sync` + `/opsx:archive` this change — **only after henk-events is archived first** (task 0.1). Ordering is not optional: these deltas MODIFY `audit-log`/`event-intake`/`incident-triage`, which enter `openspec/specs/` only when henk-events archives. Confirm with the owner before running either archive.

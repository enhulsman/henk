...
- **[`ContainerRestarting` coverage is narrower than assumed]** → ~~measured: `henk-henk-1` was recreated 8× in 15d and `changes()` never reached 1, because a recreate mints a new container ID and a new cadvisor series. So `compose up -d --build` **cannot** trip it; the rule covers in-place restart loops with a stable container ID — which still includes the case it was chosen for, a crash-looping Henk under its restart policy. Deploy-verify must use `docker restart` ×2, not a rebuild, or it verifies nothing.~~
  **FALSIFIED 2026-08-06 by task 4.6 — the mitigation above was wrong, and the rule cannot fire at all.** `docker restart` ×2 produced no alert. Cause: cadvisor's `container_start_time_seconds` is the container's **creation** time, not its last start — a 25-minute range query across two restarts returned 26 samples with **one distinct value** while `docker ps` read `Up 9 minutes`. Cross-check: `henk-henk-1`, *recreated* at 18:30:15Z, reports exactly `18:30:15Z`, so recreation updates the value and restart does not. Hence `changes() == 0` on **all three** paths — restart (constant value), recreate (new series with no prior samples), and crash loop (same container ID, so identical to restart). The last one is fatal: a crash-looping Henk was the case the rule was chosen for, and it is exactly the blind case.
  **This is the same defect class as `HenkHealthEtl` arm 4** — provisions cleanly, `health=ok`, structurally unable to fire — which is what this change existed to eliminate. `gt -1` does not help: the firing bar is irrelevant when the input never changes. The lesson generalises past D2: **restoring native firing semantics fixes rules whose *value* was wrong, not rules whose *input metric* does not carry the signal.** An invariant cannot catch this statically; only firing the rule can.
  **Resolution (decided 2026-08-06):** leave the rule deployed — it is inert, delivering nothing and costing nothing — and record the defect prominently in `monitoring.md` rather than silently dropping the coverage goal. A **follow-up change** must rebuild it on a real restart counter: this Prometheus exposes **no** metric matching `*restart*`, and cadvisor's nearest series (`container_start_time_seconds`, `container_last_seen`, `container_health_state`, `container_oom_events_total`, `container_tasks_state`) include no restart counter, so the fix needs a new source — Docker's `RestartCount` via `docker inspect -f '{{.RestartCount}}'` published through the node-exporter textfile collector, which is monotonic and readable by `increase()`. That is a metric pipeline across three hosts, not a threshold edit, so it does not belong in this change.

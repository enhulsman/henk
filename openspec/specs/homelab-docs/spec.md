# homelab-docs Specification

## Purpose
Read-only retrieval over the homelab documentation corpus, delivered to the container as a bind mount the host keeps current — never fetched by Henk. Two actions, `search` and `read`, over a section index built once per stamp change and filtered by a default-deny path allowlist at build time. Every result carries the age of the last successful pull and is marked stale past a configured bound rather than hidden, and a broken corpus registers the tool and fails honestly per call, because an absent tool leaves the agent answering documentation questions from its priors with no signal that the documentation was unreachable.
## Requirements
